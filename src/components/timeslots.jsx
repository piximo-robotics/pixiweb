...
export function Timeslots() {
  ...
  const submitSession = (e) => {
    isLoading(true)
    push(ref(db, `users/${currentUser?.uid}/timeslots/`), currentTime.toString())
    get(ref(db, `timeslots`)).then((snapshot) => {
      snapshot.forEach((childSnapshot) => {
        const date = childSnapshot.val();
        if ((new Date(date.date)).toLocaleString() === currentTime.toLocaleString()) {
 ...
 {dayComponent}
          ...
          <div class="sm:flex sm:items-start">
            {!loading ? (
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">Are you sure that you want to sign up for a session to control Pixi on {currentTime?.toLocaleDateString()} at {currentTime?.toLocaleTimeString()}?</p>
                </div>
              </div>
              </>
            ) : (
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
            ...
            <div class="mt-3 text-center mx-2 sm:text-center">
              ...
              <div class="mt-2">
                <p class="text-sm text-gray-500">You are confirmed for a session to control Pixi on {currentTime?.toLocaleDateString()} at {currentTime?.toLocaleTimeString()}. A confirmation email has been sent to {currentUser.email}.</p>
              </div>
            </div>
        </div>
